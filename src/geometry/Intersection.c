...
Point2D intersectionSegments (Point2D A1, Point2D B1, Point2D A2, Point2D B2) {
		
	float a1, b1, a2, b2;
	float x, y;
	Point2D point; point.x = -1; point.y = -1;
	...
	if(a1 != a2) {
		...
		printf("Les droites sont parrallèles : Il n'y a pas d'intersection\n");
...
Point2D intersectionSegmentCercle (Point2D A, Point2D B, Point2D origin, float ray) {
	...
	if(delta < 0)
		...
		Point2D point1; point1.x = x1; point1.y = y1;
		Point2D point2; point2.x = x1; point2.y = y1;

		Vector2D vector1 = Vector(origin, point1);
		Vector2D vector2 = Vector(origin, point2);

		float norme1 = Norm(vector1);
		float norme2 = Norm(vector2);
